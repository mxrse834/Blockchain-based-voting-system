...
const createElection = asyncHandler(async (req, res) => {
  ...
  await db.query(
    `INSERT INTO elections 
     ...
     VALUES (?, ?, ?, ?, ?)`,
    [electionId, trimmedTitle, startDate, endDate, status]
  );

  const [election] = await db.query(
    `SELECT election_id, title, start_time, end_time, status, created_at
...
const updateElection = asyncHandler(async (req, res) => {
  ...
  const updatedStartTime = startTime ? new Date(startTime) : election.start_time;
  const updatedEndTime = endTime ? new Date(endTime) : election.end_time;
  const updatedStatus = status || election.status;

  if (
    isNaN(new Date(updatedStartTime)) ||
    isNaN(new Date(updatedEndTime))
  ) {
    throw new ApiError(400, "Invalid date format");
  }
